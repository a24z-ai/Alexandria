...
interface LocalStorageData {
  libraryCards: Record<string, AlexandriaLibraryCard>;
...
export class LocalStorageReadingRecordAdapter implements ReadingRecordAdapter {
  ...
  async getLibraryCard(userId: string): Promise<StorageResult<AlexandriaLibraryCard | null>> {
    try {
      ...
      const libraryCard = data?.libraryCards[userId] || null;
      
      // Convert activeVisits back to Map if it exists
      if (libraryCard && libraryCard.activeVisits && !(libraryCard.activeVisits instanceof Map)) {
        libraryCard.activeVisits = new Map(Object.entries(libraryCard.activeVisits));
      }
    ...
    try {
      ...
      const cardToSave = {
        ...
        activeVisits: libraryCard.activeVisits instanceof Map 
          ...
          : libraryCard.activeVisits
      };
      
      data.libraryCards[libraryCard.id] = cardToSave as AlexandriaLibraryCard;
      this.saveData(data);
